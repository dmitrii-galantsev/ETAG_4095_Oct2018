...
void loop() {// This is the main function. It loops (repeats) forever.
	...
	serial.print(".");							// indicate that the antenna is being scanned
	...
	if (FastRead(DEMOD_OUT_PIN, RFcircuit, checkTime, pollTime1) == 1) {
		...
		if((timeSeconds < pastTimeSeconds + delayTime) &
				(tagNo == tagNo2) &
				(pastCircuit == RFcircuit)){	// if everything matches up, the read is a repeat - don't log it.
...
void dumpMem() {
	...
	while (fAddress < fAddressEnd) {
		...
		while ((fAddress & 0x000003FF) < 500) {	// repeat while the byte address is less than 500
			...
			for (int n = 0; n < 5;				// loop to read in an RFID code from the flash and send it out via serial comm
...
void writeRFID_To_FlashLine() {
	...
	SPI.transfer((fAddress >> 8) & 0xFF);	// second address byte
	SPI.transfer(fAddress & 0xFF);			// third address byte
	for (int n = 0; n < 6;					// loop to log the RFID code to the flash
...
void inputTime() {							// Function to set the clock
	...
	for (int n = 0; n < 13;					// loop to read all the data from the serial buffer once it is ready
